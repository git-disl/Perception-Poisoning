...
class FasterRCNN(nn.Module):
    ...
    def __init__(self, config, extractor, rpn, head, loc_normalize_mean = (0., 0., 0., 0.), loc_normalize_std = (0.1, 0.1, 0.2, 0.2)):
    ...
    def get_optimizer(self):
        ...
        for key, value in dict(self.named_parameters()).items():
            if value.requires_grad:
                if 'bias' in key:
                    params += [{'params': [value], 'lr': self.config.lr * 2, 'weight_decay': 0}]
                else:
                    params += [{'params': [value], 'lr': self.config.lr, 'weight_decay': 0.0005}]
        self.optimizer = t.optim.SGD(params)
